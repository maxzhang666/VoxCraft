    ...
    >
      <Space vertical align="start" spacing={8} style={{ width: "100%" }}>
        <Space wrap>
          ...
          <Tag color={tier.color as any}>{tier.label}</Tag>
          <Tag>{KIND_LABEL[entry.kind] ?? entry.kind}</Tag>
          <Tag color="cyan">{sizeMb} MB</Tag>
          {entry.mirror_authority === "community" && (
            <Tag color="orange">社区镜像</Tag>
          )}
          {!entry.is_builtin && <Tag color="violet">自定义</Tag>}
        </Space>

        {entry.sources.length > 0 && (
          <Text type="tertiary" size="small">
            {entry.sources.map((s) => `${s.id}: ${s.repo_id}`).join("  ·  ")}
          </Text>
        )}
        ...
        {status === "downloading" && (
          <>
            ...
            {entry.queue_position != null && entry.queue_position > 0 ? (
              <Text type="tertiary" size="small">
                ⏳ 排队中（前面还有 {entry.queue_position} 个）
              </Text>
            ) : (
              <Text type="tertiary" size="small">
                正在下载…
