...
def test_list_filter_by_kind(client):
    r = client.get("/admin/providers", params={"kind": "asr"})
    assert r.status_code == 200
    data = r.json()
    assert len(data) == 1
...
def test_update_provider(client):
    pid = client.get("/admin/providers", params={"kind": "tts"}).json()[0]["id"]
    r = client.patch(f"/admin/providers/{pid}", json={"enabled": False})
    assert r.status_code == 200
    assert r.json()["enabled"] is False


def test_set_default_mutually_exclusive(client):
    created = client.post("/admin/providers", json={
        "kind": "asr",
        "name": "alt-asr",
        "class_name": "WhisperProvider",
        "config": {"model_path": "/x"},
    }).json()
    r = client.post(f"/admin/providers/{created['id']}/set-default")
    assert r.status_code == 200
    assert r.json()["is_default"] is True

    all_asr = client.get("/admin/providers", params={"kind": "asr"}).json()
...
def test_delete_provider(client):
    ...
    r = client.delete(f"/admin/providers/{created['id']}")
    assert r.status_code == 204

    missing = client.patch(f"/admin/providers/{created['id']}", json={"enabled": True})
...
def test_test_endpoint_succeeds_for_loadable_provider(client, mock_asr_registered):
    ...
    r = client.post(f"/admin/providers/{created['id']}/test")
    assert r.status_code == 200
    body = r.json()
...
def test_list_classes_filter_by_kind(client):
    r = client.get("/admin/providers/classes", params={"kind": "separator"})
    assert r.status_code == 200
    data = r.json()
...
def test_test_endpoint_reports_failure_when_model_missing(client):
    ...
    pid = client.get("/admin/providers", params={"kind": "asr"}).json()[0]["id"]
    r = client.post(f"/admin/providers/{pid}/test")
    assert r.status_code == 200
    body = r.json()
