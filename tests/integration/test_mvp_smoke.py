...
def _create_mock_set(client):
    ...
    for kind, name, cls in specs:
        ...
        ids[kind] = r.json()["id"]
        assert client.post(f"/api/admin/providers/{ids[kind]}/set-default").status_code == 200
    return ids
...
def test_mvp_smoke_full_flow(client, mock_all_registered):
    ...
    # 5. /tts
    r_tts = client.post("/api/tts", json={"text": "你好", "voice_id": "mock-voice"})
    ...
    # 15. 删除 Job → 产物链同步失效
    assert client.delete(f"/api/jobs/{tts_job_id}").status_code == 204
    assert client.get(f"/api/jobs/{tts_job_id}").status_code == 404
...
def test_mvp_smoke_error_paths(client, mock_all_registered):
    ...
    # 3. Job 不存在
    r3 = client.get("/api/jobs/nonexistent")
...
def test_retry_failed_job(client, mock_all_registered, monkeypatch):
    ...
    r = client.post(
        "/api/asr", files={"audio": ("a.wav", b"RIFFfake", "audio/wav")},
        data={"language": "zh"},
    )
    ...
    r2 = client.post(f"/api/jobs/{job_id}/retry")
    ...
    r3 = client.post(f"/api/jobs/{job_id}/retry")
...
def test_mvp_smoke_models_library_flow(client, monkeypatch):
    ...
    # 1. catalog 列表含 9 内置（cloning 下线后 + Moonshine 集成后）
    ...
    # 3. 等 ready
    for _ in range(60):
        vs = client.get("/api/admin/models-library").json()
    ...
    # 6. 删除 ready 模型
    r4 = client.delete(f"/api/admin/models-library/{mid}")
