...
from voxcraft.runtime.gpu import resolve_device
...
def run(
    ...
    """
    # 解析实际生效设备并打日志；同步注入到返回 result.device 给前端展示
    device = resolve_device(req.provider_config.get("device"))
    log.info(
        "job.run.start",
        job_id=req.job_id,
        kind=req.kind,
    ...
    try:
        ...
        log.info(
            "job.run.done",
            job_id=req.job_id, kind=req.kind, device=device, ok=result.ok,
        )
        return replace(result, result=merged)
